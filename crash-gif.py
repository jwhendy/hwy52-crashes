...
# when we save the screenshots, they live in the ./gif sub directory; create it!
browser.save_screenshot('{}/gif/{}.png'.format(path, '0-0-0'))
...
for date in df_decay.date.unique():
    ...
    browser.get('file://{}/map.html'.format(path))
    time.sleep(1)
    browser.save_screenshot('{}/gif/{}.png'.format(path, pd.to_datetime(date).strftime('%Y-%m-%d')))
...
# - originally tried to scale the days between crashes to a target gif length
...
cmd_list = ['convert', '-loop', '1', '-delay', '50', './gif/0-0-0.png']
for i, row in df_gif.iterrows():
    # crashes on that day, flash for 1 day
    ...
    if row.date=='2010-12-05':
        ...
        cmd_list.append('1')
    cmd_list.append('./gif/{}.png'.format(row.date))

cmd_list.extend(['./gif/crashes.gif'])
